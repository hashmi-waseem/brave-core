...
void RewardsServiceImpl::CheckPreferences() {
  ...
  // the "search ads enabled" pref has the appropriate default value.
  if (prefs->GetBoolean(prefs::kEnabled) &&
      !prefs->GetString(prefs::kExternalWalletType).empty() &&
      !prefs->GetUserPrefValue(brave_ads::prefs::kOptedInToSearchResultAds)) {
...
void RewardsServiceImpl::ExternalWalletConnected() {
  // When the user connects an external wallet, turn off search ads since users
  // cannot earn BAT for them yet. The user can turn them on manually.
